...
var clusterEvents = []fsm.EventDesc{
	{Name: string(ConnectCluster), Src: []string{dbapi.ConnectorClusterPhaseDisconnected, dbapi.ConnectorClusterPhaseReady}, Dst: dbapi.ConnectorClusterPhaseReady},
	{Name: string(ConnectCluster), Src: []string{dbapi.ConnectorClusterPhaseDeleting}, Dst: dbapi.ConnectorClusterPhaseDeleting},
	{Name: string(DisconnectCluster), Src: []string{dbapi.ConnectorClusterPhaseDisconnected, dbapi.ConnectorClusterPhaseReady}, Dst: dbapi.ConnectorClusterPhaseDisconnected},
	{Name: string(DeleteCluster), Src: []string{dbapi.ConnectorClusterPhaseDisconnected, dbapi.ConnectorClusterPhaseReady, dbapi.ConnectorClusterPhaseDeleting}, Dst: dbapi.ConnectorClusterPhaseDeleting},
}

func NewClusterFSM(cluster *dbapi.ConnectorCluster) *ClusterFSM {
	return &ClusterFSM{
		Cluster: cluster,
		fsm:     fsm.NewFSM(cluster.Status.Phase, clusterEvents, nil),
	}
}
...
func (c *ClusterFSM) Perform(operation ClusterOperation) (bool, *errors.ServiceError) {
	...
	c.fsm.SetState(c.Cluster.Status.Phase)
	if err := c.fsm.Event(string(operation)); err != nil {
		switch err.(type) {
		case fsm.NoTransitionError:
			return false, nil
		default:
			return false, errors.BadRequest("Cannot perform operation %q on Cluster because %s",
				operation, err)
		}
	}

	c.Cluster.Status.Phase = c.fsm.Current()
	return true, nil
}
